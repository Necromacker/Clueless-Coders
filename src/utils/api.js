...
export const fetchFoodNutrients = async (query) => {
    const url = `${BASE_URL}/recipedb/recipeByTitle?title=${encodeURIComponent(query)}`;
    console.log(`[API REQUEST] fetchFoodNutrients: ${url}`);
    try {
        const response = await fetch(url, {
            headers: { 'Authorization': `Bearer ${API_KEY}` }
        });
        const data = await response.json();
        console.log(`[API RESPONSE] fetchFoodNutrients:`, data);
        return data[0];
    } catch (error) {
...
export const fetchFlavorMatch = async (foodPair) => {
    const url = `${BASE_URL}/flavordb/food/by-alias?food_pair=${encodeURIComponent(foodPair)}`;
    console.log(`[API REQUEST] fetchFlavorMatch: ${url}`);
    try {
        const response = await fetch(url, {
            headers: {
        ...
        const data = await response.json();
        console.log(`[API RESPONSE] fetchFlavorMatch:`, data);
        return data;
    } catch (error) {
...
export const fetchReplacement = async (alias) => {
    const url = `${BASE_URL}/flavordb/food/by-alias?alias=${encodeURIComponent(alias)}`;
    console.log(`[API REQUEST] fetchReplacement: ${url}`);
    try {
        const response = await fetch(url, {
            headers: { 'Authorization': `Bearer ${API_KEY}` }
        });
        const data = await response.json();
        console.log(`[API RESPONSE] fetchReplacement:`, data);
        return data;
    } catch (error) {
