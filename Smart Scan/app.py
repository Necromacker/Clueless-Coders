...
def identify_food():
    print(f"\n[BACKEND REQUEST] /identify")
    print(f"Files: {list(request.files.keys())}")
    print(f"Form Data: {request.form}")

    if not GOOGLE_API_KEY:
         print(f"[BACKEND ERROR] Gemini API Key not configured")
         return jsonify({'error': 'GEMINI_API_KEY not configured in .env file'}), 500

    if 'image' not in request.files:
        print(f"[BACKEND ERROR] No image file provided")
        return jsonify({'error': 'No image file provided'}), 400
    
    file = request.files['image']
    if file.filename == '':
        print(f"[BACKEND ERROR] No selected file")
    ...
    try:
        ...
        if response.text:
            ...
            try:
                ...
                print(f"[BACKEND RESPONSE] /identify: {json.dumps(response_data, indent=2)}")
                return jsonify(response_data)

            except json.JSONDecodeError:
                print(f"[BACKEND ERROR] Failed to parse Gemini response: {cleaned_text}")
                return jsonify({'error': 'Failed to parse API response', 'raw': cleaned_text}), 500
            except Exception as e:
                 print(f"[BACKEND ERROR] Processing data: {str(e)}")
                 return jsonify({'error': f'Error processing data: {str(e)}'}), 500
        else:
             print(f"[BACKEND ERROR] Gemini returned no text")
